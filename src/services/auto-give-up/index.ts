...
export default class AutoGiveUpService {
  ...
  private static isTimeout(pickedAt: string, timeout: number): boolean {
    return (
      new Date().valueOf() - Date.parse(pickedAt) >
      timeout * 24 * 60 * 60 * 1000
    );
  }

  private async findAllNonePullChallengeIssues(
    autoGiveUpQuery: AutoGiveUpQuery
  ): Promise<ChallengeIssue[]> {
    const challengeIssues = await this.challengeIssueRepository
      .createQueryBuilder("ci")
      .leftJoinAndSelect("ci.issue", "issue")
      .where(
        "issue.owner = :owner and issue.repo = :repo and ci.has_picked = 1",
        { ...autoGiveUpQuery }
      )
    ...
    return challengeIssues.filter((c) => {
      ...
      return (
        pickedAt &&
        c.challengePulls &&
        c.challengePulls.length === 0 &&
        AutoGiveUpService.isTimeout(
          pickedAt,
  ...
  public async autoGiveUp(
    autoGiveUpQuery: AutoGiveUpQuery
  ): Promise<AutoGiveUpResult[]> {
    const result: AutoGiveUpResult[] = [];
    const challengeIssues = await this.findAllNonePullChallengeIssues(
      autoGiveUpQuery
    );
