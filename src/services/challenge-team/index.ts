...
export default class ChallengeTeamService {
  ...
  public async create (challengeTeamQuery: ChallengeTeamQuery): Promise<Response<ChallengeTeam|null>> {
    ...
    const alreadyTeamedChallengers = await this.getChallengersInAnotherTeam(challengers, program.id)
    ...
    const challengersChallengeTeams:ChallengersChallengeTeams[] = challengers.map(c => {
    ...
    await this.challengersChallengeTeamsRepository.save(challengersChallengeTeams)
